...
const int BACK_RIGHT_SPEED = BASE_SPEED + 50; 

const int SOFT_LOW_SPEED = BASE_SPEED - 90;  
const int RECOVERY_SPIN_SPEED = 140; 

// Asymmetric Backup Speeds (Left side fast, Right side slow)
const int BACKUP_SPEED_FAST = 150; 
const int BACKUP_SPEED_SLOW = 70;  

// Tuning parameters for 11110 / 01111 extreme turns
const int SHARP_FORWARD_SPEED = 200; 
const int SHARP_REVERSE_SPEED = 120; 

// Junction Turn Settings
const int JUNCTION_CW_FAST = 230; 
const int JUNCTION_CW_SLOW = 150; 
const int TURN_DURATION    = 4000; 
...
void setup() {
  Serial.begin(9600);
  Serial.println(F("===================================="));
  Serial.println(F(" LFR: DIRECTION VERIFIED CODES v7   "));
  Serial.println(F("===================================="));
...
void loop() {
  ...
  if (allSensorsOnBlack()) {
    Serial.println(F("-> [JUNCTION] Triggering Asymmetric Reverse"));
    executeJunctionClockwise();
    return; 
  }
  ...
  else if (sensorPattern == "00111" || sensorPattern == "01111") { 
  ...
  switch (currentState) {
    ...
    case STATE_R:  
      executePivotMotors(HIGH, LOW, LOW, HIGH, SHARP_FORWARD_SPEED, SHARP_REVERSE_SPEED);
      break;
...
void executeJunctionClockwise() {
  stopMotors();
  delay(250); 

  // ===================================================================
  // TROUBLESHOOTING BACKUP DIRECTION:
  // If the robot moves FORWARD here instead of backward, change:
  // Left Side:  HIGH, LOW  instead of LOW, HIGH
  // Right Side: HIGH, LOW  instead of LOW, HIGH
  // ===================================================================
  
  // Left Side BACKWARD (Fast)
  ...
  // Dynamic loop: Run until the line sensors pick up the cross-line again
  unsigned long safetyTimeout = millis();
  while (!allSensorsOnBlack()) {
    if (millis() - safetyTimeout > 2000) {
      break; 
    }
  }
  ...
  // Clockwise Turn Execution (Left Side Forward, Right Side Backward)
  executePivotMotors(HIGH, LOW, LOW, HIGH, RECOVERY_SPIN_SPEED, RECOVERY_SPIN_SPEED);
  delay(TURN_DURATION); 
