...
// Junction Turn Settings (Reconfigured for Clockwise Pivot)
const int JUNCTION_CW_FAST = 230; // Left wheels forward fast
const int JUNCTION_CW_SLOW = 150; // Right wheels backward fast/steady
const int TURN_DURATION    = 2500; // Adjust this time (ms) to hit your target angle
...
void setup() {
  Serial.begin(9600);
  Serial.println(F("===================================="));
  Serial.println(F(" LFR: JUNCTION -> CLOCKWISE PIVOT    "));
  Serial.println(F("===================================="));
...
void loop() {
  ...
  if (allSensorsOnBlack()) {
    Serial.println(F("-> [JUNCTION] Triggering Clockwise Pivot Sequence"));
...
// Swapped Left Turn with Clockwise Rotation sequence
void executeJunctionClockwise() {
  stopMotors();
  delay(350); 

  // Back up slightly from the intersection
  executePivotMotors(LOW, HIGH, LOW, HIGH, BACKUP_SPEED, BACKUP_SPEED);
  delay(300); 

  // NEW: Clockwise Turn Execution (Left Side Forward, Right Side Backward)
  executePivotMotors(HIGH, LOW, LOW, HIGH, RECOVERY_SPIN_SPEED, RECOVERY_SPIN_SPEED);
  delay(TURN_DURATION); 
  ...
  lastDirection = DIR_RIGHT; // Treat the clockwise spin as a right-hand history tracking point
  currentState = STATE_F; 
}
